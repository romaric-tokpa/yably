-- PostGIS : index spatial + filtre ST_DWithin pour les pharmacies de garde.
-- Sur Supabase hébergé : activer l’extension « postgis » (SQL Editor ou Dashboard → Extensions) si cette ligne échoue.
...
CREATE INDEX IF NOT EXISTS idx_pharmacies_geom_gist
  ON public.pharmacies USING GIST (geom);

CREATE OR REPLACE FUNCTION public.pharmacies_sync_geom()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_pharmacies_geom ON public.pharmacies;
CREATE TRIGGER trg_pharmacies_geom
  BEFORE INSERT OR UPDATE OF latitude, longitude ON public.pharmacies
  FOR EACH ROW
  EXECUTE FUNCTION public.pharmacies_sync_geom();
